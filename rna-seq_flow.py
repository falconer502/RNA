...
def build_index(genome,gtf):
	extract_es(gtf)
	mkdir('index')
	cmd='hisat2-build -f %s --ss splicesites.txt --exon exons.txt -p 7 ./index/genome.index' %genome
...
def align_genome(genome,fa_path):   #align genome,input path of data files
	...
	for i in sequenced_file:
		if os.path.isdir(fa_path+i)==True:
			...
			cmd='hisat2 -q -x ./index/genome.index -1 %s -2 %s -S ./temp/%s.sam -t'%(file1,file2,i)
	...
	for i in os.listdir(filepath):
		if '.sam' in i: #i==t01.sam t02.sam t01.sam.sorted.bam
			...
			if 'merged.gtf' in gtf :
				...
				exec('samtools view -Su ./temp/%s | samtools sort > ./temp/%s.sorted.bam'%(name,name),'converted to bam format using samtools')
